document.addEventListener("DOMContentLoaded", () => {
  ...
  const calendarIcons = document.querySelectorAll(
    ".field-checkin .fa-calendar, .field-checkout .fa-calendar"
  );
  ...
  const createCalendarElement = (inputElement) => {
    const existingCalendar =
      inputElement.parentElement.querySelector(".calendar-popup");
  ...
  const renderCalendar = (calendarDiv, date, inputElement) => {
    ...
    let minDate = parseIsoDate(minDateStr);
    if (minDate) minDate.setHours(0, 0, 0, 0);
    let maxDate = parseIsoDate(maxDateStr);
  ...
  const setupCalendar = (icon) => {
    icon.addEventListener("click", (e) => {
      ...
      document.querySelectorAll(".calendar-popup").forEach((c) => {
  ...
  document.addEventListener("click", (e) => {
    if (
      !e.target.closest(".field-checkin") &&
      !e.target.closest(".field-checkout")
    ) {
      document
  ...
  const renderRooms = (rooms, checkIn, checkOut, nights) => {
    roomBox.innerHTML = ""; // empty old card
    ...
    rooms.forEach((room) => {
        // create room-card for each room
      ...
      roomBox.appendChild(card); //add node

      // temp storage for room-card selection
      ...
      bookBtn.addEventListener("click", () => {
        ...
        localStorage.setItem(
          "stay_guests",
          `${roomsVal} Room(s) - ${
            parseInt(adultsVal) + parseInt(childrenVal)
          } Guests`
        );
  ...
  if (searchBtn) {
    searchBtn.addEventListener("click", async (e) => {
      ...
      const roomsVal = document.getElementById("select-rooms").value;
      const adultsVal = parseInt(
        document.getElementById("select-adults").value
      );
      const childrenVal = parseInt(
      ...
      try {
        const res = await fetch(
          "http://localhost:8080/api/reservation/search",
          {
            method: "POST",
        ...
        const nights =
          (parseDisplayDate(checkOutVal) - parseDisplayDate(checkInVal)) /
          (1000 * 60 * 60 * 24);
